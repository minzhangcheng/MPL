...
def insertQuery(table, insertValueDict):
    c, v = Misc.dict2Lists(insertValueDict)
...
def insertManyQuery(table, insertValueListDict, itemPerOneTime=500):
    queries = list()
    c, v = Misc.dict2Lists(insertValueListDict)
    c = ', '.join(c)  # c: string for columns
    vl = Misc.Table.transpose(v)
    begin = 0
    q = "INSERT INTO %s (%s) VALUES\n" % (table, c)
